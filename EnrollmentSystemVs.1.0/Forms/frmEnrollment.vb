Public Class frmEnrollment
    ...
    Private Sub dtgList_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles dtg_list.CellClick
        sql = "SELECT `IDNO`, CONCAT( `LNAME`,', ', `FNAME`,' ', `MNAME`) as 'FULL NAME', `SEX`" _
              & ", `AGE`, `HOME_ADD` as 'ADDRESS'" _
              & " FROM `tblstudent` WHERE `IDNO` = '" & dtg_list.CurrentRow.Cells(0).Value & "'"
        fillSpecifictxt(sql)
        Try
            txtFullname.Text = dt.Rows(0).Item("FULL NAME")
            'for the replica of LEVEL and SCHOOL YEAR 
            sql = " SELECT * FROM `tblstudupdatelevel` " _
            & "WHERE `IDNO` ='" & dtg_list.CurrentRow.Cells(0).Value & "'"
            fillSpecifictxt(sql)
    ...
    Private Sub btnClear_Click(ByVal sender As System.Object, ByVal e As System.EventArgs)
        ...
        fillGrid(sql, dtg_EnrollmentHistory)
    ...
    Private Sub btnSave_Click(ByVal sender As System.Object, ByVal e As System.EventArgs)
        Dim id As String = dtg_list.CurrentRow.Cells(0).Value
        Dim Level As String
        Try
    ...
    Private Sub tsSearch_TextChanged(sender As Object, e As EventArgs) Handles tsSearch.TextChanged
        sql = "SELECT s.`IDNO`, CONCAT(`FNAME` ,' ', `LNAME`,' ', `MNAME`) as 'FULL NAME', `SEX` , `AGE`, `HOME_ADD` as 'ADDRESS' ,`LEVEL` , `SECTION`  , `AY` " &
  " FROM `tblstudent` s, `tblstudupdatelevel` sl WHERE s.`IDNO`=sl.`IDNO` AND ( CONCAT(`FNAME` ,' ', `LNAME`,' ', `MNAME`) LIKE '%" & tsSearch.Text & "%' OR FNAME LIKE '%" & tsSearch.Text &
  "' OR LNAME LIKE '%" & tsSearch.Text & "' OR LEVEL LIKE '%" & tsSearch.Text & "' OR SECTION LIKE '%" & tsSearch.Text & "' OR AY LIKE '%" & tsSearch.Text & "' OR CONCAT(`LEVEL` ,' ', `AY`) LIKE '%" & tsSearch.Text & "' OR CONCAT(`LEVEL` ,' ', `SECTION`) LIKE '%" & tsSearch.Text & "' OR CONCAT(`LNAME` ,' ', `AY`) LIKE '%" & tsSearch.Text & "')"
        fillGrid(sql, dtg_EnrollmentHistory)
    ...
    Private Sub dtgList_Click(sender As Object, e As DataGridViewCellEventArgs) Handles dtg_list.CellClick
